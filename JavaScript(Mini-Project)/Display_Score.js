...
function levelUp(){
    userSeq=[];//jese hi levelup function call hota h hm userseq ko khali kr denge or vaaps se reset ho jaayega
    ...
    console.log(gameSeq);
    gameFlash(randBtn);//gameseq me naya random color add ho rha h
...
//current level ko pass krenge hr level pe track krenge
function checkAns(idx){
    // console.log("curr level: ",level);
    //let idx= level-1;
    if(userSeq[idx]===gameSeq[idx]){
        if(userSeq.length==gameSeq.length){//console.log("same value");
         setTimeout(levelUp,1000);//1s ka delay hoga levelup me
        }
        }else{
        h2.innerHTML=`Game Over! Your score was<b>${level}</b><br>Press any key to start,`;
        document.querySelector("body").style.backgroundColor="red";//jese hi glt press krte ho or gameover hota h puri screen red dikhegi kuch time ke liye
        setTimeout(function(){
            document.querySelector("body").style.backgroundColor="white";
        },200);
        reset();//game over hone ke baad vaaps hm reset kr denge game ko
    }
}
...
function btnPress(){
    ...
    userColor = btn.getAttribute("id");//specific color ko id denge to vo uski value btaayega
